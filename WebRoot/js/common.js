...
function addToCartShowDivResponse(result)
{
  ...
  else
  {
	...
	{
		$("#MyDiv1 .center_pop_txt2").html(result.show_info);
		$("#MyDiv1").show();	
		$("#fade1").show();
	}
  }
...
function addToCart_bak(goodsId, parentId)
{
  ...
  Ajax.call('flow.php?step=add_to_cart', 'goods=' + $.toJSON(goods), addToCartResponse_bak, 'POST', 'JSON');
}
...
function addToCartResponse_bak(result)
{
  ...
  else
  {
    ...
    {
      cartInfo.innerHTML = result.content;
      //document.getElementById('nav_cart_info').innerHTML = result.goods_number;
    }
